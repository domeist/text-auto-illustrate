...
public class TrecDocIndexer {
    ...
    static final public String FIELD_IMAGE = "image";
    ...
    static Field constructImageField(String image) {
        FieldType imageFieldType = new FieldType();
        imageFieldType.setIndexOptions(IndexOptions.DOCS_AND_FREQS);
        imageFieldType.setStored(true);    // default = false (same as Field.Store.NO)
        imageFieldType.setTokenized(true);  // default = true (tokenize the content)
        imageFieldType.setOmitNorms(false); // default = false (used when scoring)
        Field imageField = new Field(FIELD_IMAGE, image, imageFieldType);
        return imageField;
    }
    ...
    Document constructDoc(String id, String image, String content) throws IOException {

        Field idField = constructIDField(id);
        Field imageField = constructImageField(image);
    ...
    void indexFindexFileWithLineReaderSimple(InputStream is) throws Exception {
        ...
        while ((line = br.readLine())!= null) {
            ...
            if (parts.length >= 4) {
                doc = constructDoc(parts[0], parts[2], parts[3]);
    ...
    void indexFileWithLineReader(InputStream is) throws Exception {
        ...
        while ((line = br.readLine())!= null) {
            ...
            if (line.startsWith("<pno>")) {
                ...
                doc = constructDoc(id, contentBuff.toString(), contentBuff.toString());
                writer.addDocument(doc);
    ...
    void indexFileWithDOM(InputStream is) throws Exception {
        ...
        for (Element docElt : docElts) {
            ...
            doc = constructDoc(docIdElt.text(), docElt.text(), docElt.text());
            writer.addDocument(doc);
        }
    }
