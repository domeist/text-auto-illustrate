...
- **Charts in the README**, generated by `tools/plot_results.py` from the sweep
  CSVs and committed to `docs/`.
- **Maven wrapper**, so a JDK is the only prerequisite.
- **Demo corpus covering both evaluation sets**, not just the strict one.
